...
const EmployeesPage = () => {
  ...
  const [formData, setFormData] = useState({ name: '', email: '', property: '', department: '', level: 'Staff', position: '', supervisor: '' });
  ...
  const handleAdd = () => {
    setModalMode('add');
    setFormData({ name: '', email: '', property: '', department: '', level: 'Staff', position: '', supervisor: '' });
    setShowModal(true);
  };
  ...
  const handleEdit = (emp) => {
    ...
    setFormData({ name: emp.name, email: emp.email, property: emp.property, department: emp.department, level: emp.level, position: emp.position || '', supervisor: emp.supervisor || '' });
    setShowModal(true);
  };
  ...
  return (
    <div>
      <PageHeader
        ...
        actions={<Button onClick={handleAdd} variant="primary"><Plus className="w-4 h-4 mr-2"/>Add Employee</Button>}
      />

      <div className="p-8 space-y-6">
        <div className="grid grid-cols-4 gap-6">
          <StatCard title="Total Karyawan" value={employees.length} />
          <StatCard title="Staff" value={employees.filter(e => e.level === 'Staff').length} valueColor="text-sky-600" />
          <StatCard title="Leader/Supervisor" value={employees.filter(e => e.level === 'Leader' || e.level === 'Supervisor').length} valueColor="text-amber-600" />
          <StatCard title="Managerial" value={employees.filter(e => e.level === 'Managerial').length} valueColor="text-purple-600" />
        </div>

        <SearchBar value={searchTerm} onChange={setSearchTerm} placeholder="Cari nama atau email..." />
                ...
                <tr key={emp.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center justify-end space-x-2">
                      <Button variant="ghost" size="sm" onClick={() => handleEdit(emp)}><Edit2 className="w-4 h-4"/></Button>
      ...
      <Modal isOpen={showModal} onClose={() => setShowModal(false)} title={modalMode === 'add' ? 'Add New Employee' : 'Edit Employee'} footer={<>
        <Button variant="ghost" onClick={() => setShowModal(false)}>Cancel</Button>
        <Button variant="primary" onClick={handleSave}><Save className="w-5 h-5 mr-2"/>{modalMode === 'add' ? 'Add Employee' : 'Save Changes'}</Button>
      </>}> 
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="block mb-2 text-sm font-medium text-gray-700">Full Name</label>
            <Input type="text" value={formData.name} onChange={(e) => setFormData({...formData, name: e.target.value})} placeholder="e.g. Budi Santoso" />
          </div>
          <div>
            <label className="block mb-2 text-sm font-medium text-gray-700">Email</label>
            <Input type="email" value={formData.email} onChange={(e) => setFormData({...formData, email: e.target.value})} placeholder="e.g. budi@lpphotel.com" />
          </div>
        </div>
        <div className="grid grid-cols-2 gap-4 mt-4">
          <div>
            <label className="block mb-2 text-sm font-medium text-gray-700">Property</label>
            <select value={formData.property} onChange={(e) => setFormData({...formData, property: e.target.value, department: ''})} className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-500">
          ...
          <div>
            ...
            <select value={formData.department} onChange={(e) => setFormData({...formData, department: e.target.value})} className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-500" disabled={!formData.property}>
              <option value="">Pilih Department</option>
              {getAvailableDepartments().map((dept, idx) => (<option key={idx} value={dept}>{dept}</option>))}
            </select>
          </div>
        </div>
        <div className="grid grid-cols-2 gap-4 mt-4">
          ...
          <div>
            ...
            <Input type="text" value={formData.position} onChange={(e) => setFormData({...formData, position: e.target.value})} placeholder="e.g. FO Staff, HK Supervisor" />
          </div>
        </div>
        <div className="mt-4">
          <label className="block mb-2 text-sm font-medium text-gray-700">Atasan</label>
          <Input type="text" value={formData.supervisor} onChange={(e) => setFormData({...formData, supervisor: e.target.value})} placeholder="e.g. Ratna Sari" disabled={formData.level === 'GM'} />
